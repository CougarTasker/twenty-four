...
class Background:
	def __init__(self,dimensions,time):
        ...
        #draws main wave (front wave in layers) as polygon
	...
	#drawa wave polygons to the screen
	...
	def draw(self, canvas):
		...
		pollycount = 35
		self.background(canvas,pollycount,2,0.2,0.3,0.05,"rgb(0,0,250)")#draws the front wave as a full polygon
		...
		self.draw_wave_parts(canvas,middles,"rgb(0,0,150)")#draw these wave sections on behind main polygon
		self.draw_wave_parts(canvas,smalls,"rgb(0,0,100)")#draw the smaller waves sections 
